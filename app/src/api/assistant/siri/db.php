...
class Database extends Config
{
	...
	public function updateAccionChapa($num_serie, $id_usuario, $accion, $token)
	{
		...
		if($result){
			...
			$sql = 'SELECT token from chapa WHERE ch_num_serie = :num_serie';
	    	$stmt = $this->conn->prepare($sql);
			$stmt->execute(['num_serie' => $num_serie]);
	    	$rows = $stmt->fetchAll();

			if(count($rows) == 1 
				&& isset($rows[0]["token"]) 
				&& strcmp($rows[0]["token"],$token) == 0
				){
